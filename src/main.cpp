...
static bool create_swapchain(VulkanContext& ctx, uint32_t w, uint32_t h) {
    ...
    if (vkCreateSwapchainKHR(ctx.device, &ci, nullptr, &new_swapchain) != VK_SUCCESS)
        return false;
    ...
    vkGetSwapchainImagesKHR(ctx.device, ctx.swapchain, &count, ctx.swapchain_images.data());
...
static bool init_vulkan(VulkanContext& ctx, SDL_Window* window) {
    ...
    inst_ci.enabledExtensionCount = static_cast<uint32_t>(ctx.enabled_instance_extensions.size());
    ...
    dev_ci.enabledExtensionCount = static_cast<uint32_t>(ctx.enabled_device_extensions.size());
    dev_ci.ppEnabledExtensionNames = ctx.enabled_device_extensions.data();

    if (vkCreateDevice(ctx.physical_device, &dev_ci, nullptr, &ctx.device) != VK_SUCCESS)
        return false;
    ...
    if (!create_swapchain(ctx, static_cast<uint32_t>(w), static_cast<uint32_t>(h)))
        return false;
...
int main(int argc, char** argv) {
    ...
    // Skia Vulkan context
    skgpu::VulkanGetProc get_proc = [](const char* name, VkInstance inst, VkDevice dev) -> PFN_vkVoidFunction {
        if (dev)
    ...
    skia_vk_exts.init(get_proc, vk.instance, vk.physical_device,
                      static_cast<uint32_t>(vk.enabled_instance_extensions.size()), vk.enabled_instance_extensions.data(),
                      static_cast<uint32_t>(vk.enabled_device_extensions.size()), vk.enabled_device_extensions.data());
    ...
    SkColorType sk_color_type = (vk.swapchain_format == VK_FORMAT_B8G8R8A8_UNORM
    ...
    while (running) {
        ...
        if (needs_resize) {
            ...
            if (w > 0 && h > 0) {
                ...
                if (!create_swapchain(vk, static_cast<uint32_t>(w), static_cast<uint32_t>(h))) {
        ...
        VkResult acquire_result = vkAcquireNextImageKHR(
            vk.device, vk.swapchain, UINT64_MAX, vk.image_available, VK_NULL_HANDLE, &image_index
        );
        ...
        GrBackendSemaphore wait_sem = GrBackendSemaphores::MakeVk(vk.image_available);
        ...
        GrBackendSemaphore signal_sem = GrBackendSemaphores::MakeVk(vk.render_finished);
        auto present_state = skgpu::MutableTextureStates::MakeVulkan(
            VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, vk.queue_family
        );
